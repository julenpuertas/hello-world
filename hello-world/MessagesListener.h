...
namespace Engine
{
	namespace Messages
	{
		...
		class Listener
		{
			...
			MultiHashMap<std::type_index, std::shared_ptr<Handler> > private_handlers_;
			DynamicArray<Pair<std::type_index, std::weak_ptr<Handler> > > public_handlers_;

			static void add_public_handler(const std::type_info& type, const std::shared_ptr<Handler>& p_handler);
		...
		public:
			...
			template <typename T, typename LambdaT> std::weak_ptr<Handler> add_private_handler(LambdaT&& callback);
			...
			template <typename T, typename LambdaT> std::weak_ptr<Handler> add_public_handler(LambdaT&& callback);
